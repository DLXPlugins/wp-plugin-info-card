...
jQuery( document ).ready( function( $ ) {
	...
	const wrapper = '.wppic-list';	 		//Fields wrapper
	const add_button = $( '.wppic-add-fields' ); 	//Add button ID
	const add_input = '.wppic-add'; 			//Fields list
	const field_remove = 'wppic-remove-field';		//Remove item
	...
	$( '.' + field_remove ).on( 'click', function() { //remove field
	...
	if ( $( '#wppic-dashboard-widget' ).length > 0 && $( '#wppic-dashboard-widget .wp-pic-loading' ).length > 0 && $( '#wppic-dashboard-widget .wp-pic-widget-empty' ).length == 0 ) {
		$( '.wp-pic-list .wp-pic-loading' ).each( function() {
			...
			const dataList = elem.data( 'list' );
			const listLength = $.map( dataList, function( n, i ) {
				return i;
			} ).length;
			let count = 1;
			...
			$.each( dataList, function( index, value ) {
				const data = {
					...
					'wppic-type': dataType,
					'wppic-list': value,
					nonce: wppic_dashboard_widget.nonce,
				};
				$.post( ajaxurl, data, function( response ) {
					...
					for ( let i = 0; i < listLength; i++ ) {
						const $current = $( '.wp-pic-list .wp-pic-item[data-index="' + i + '"]' );
						if ( $current.length > 0 ) {
							$current.fadeIn( 500 );
						} else {
							break;
	...
	if ( $( '.wppic-cache-clear-button.first' ).length > 0 ) {
		$( '.wppic-cache-clear-button.first' ).on( 'click', function() {
			...
			$.ajax( {
				...
				success( response ) {
					const defaultText = $( '.wppic-cache-clear-button' ).html();
