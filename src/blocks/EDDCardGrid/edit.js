...
const SitePluginsCardGrid = ( props ) => {
	...
	const loadPlugins = ( page = 1 ) => {
		...
		setLoading( true );
		const restUrl = wppic.rest_url + 'wppic/v2/get_edd_plugins';
		axios
			.get(
				restUrl,
				{
	...
	const outputInfoCards = () => {
		return Object.values( plugins ).map( ( cardData, key ) => {
			return (
				<Fragment key={ key }>
					{ 'flex' === layout && (
						<PluginFlex
	...
	const blockProps = useBlockProps( {
		className: classnames( `edd-plugins-grid align${ align }` ),
	} );
	...
	/**
	 * Return loading placeholder if in initial loading state.
	 */
	const loadingPlaceholder = (
		<div className="wppic-site-plugins-block wppic-site-plugins-panel" { ...blockProps }>
			<div className="wppic-block-svg">
				<Logo size="75" />
			</div>
			<div className="wppic-site-plugins-description">
				<p>
					{ __( 'Loading your plugins…', 'wp-plugin-info-card' ) }
				</p>
			</div>
			<Spinner />
		</div>
